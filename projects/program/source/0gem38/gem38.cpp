...
static void initializeDyeField(vec2u const& dims, std::vector<vec4f>& field) {
    mark();
    field.resize(dims.x * dims.y);
    vec2f dimsf = vec2f{ dims.x, dims.y };
    ...
    for(u32 j = 0; j < dims.y; ++j) {
        for(u32 i = 0; i < dims.x; ++i) {
            field[i + j * dims.x] = vec4f{0.0f, 1.0, 0.5f, 1.0f};
        }
    }
    mark();
    return;
...
static std::vector<vec4f> g_dyeField;
...
static u32                g_fbo;
static ShaderProgramV2    g_compute[8];


static ShaderProgramV2& gr_computeboundary = g_compute[0];
static ShaderProgramV2& gr_compute0 = g_compute[1];
static ShaderProgramV2& gr_compute1 = g_compute[2];
static ShaderProgramV2& gr_compute2 = g_compute[3];
static ShaderProgramV2& gr_compute3 = g_compute[4];
static ShaderProgramV2& gr_compute4 = g_compute[5];
static ShaderProgramV2& gr_compute5 = g_compute[6];
static ShaderProgramV2& gr_computedraw = g_compute[7];

static u32&             gr_vel_pressure = g_texture[0];
static u32&             gr_extern_force = g_texture[1];
static u32&             gr_diffusion    = g_texture[2];
static u32&             gr_fieldw       = g_texture[3];
static u32&             gr_scratch_tex0 = g_texture[5];
static u32&             gr_scratch_tex1 = g_texture[6];
static u32&             gr_scratch_tex2 = g_texture[7];
static u32&             gr_dye          = g_texture[8];
...
i32 gpugems38_main()
{
    ...
    bool ok = true;
    for(u8 i = 0; ok && i < __carraysize(computeShaderFilename); ++i) {
...
static void destroyTextures()
{
    gl::glDeleteTextures(8, &g_texture[0]);
...
static void renderToDrawTexture()
{
    ...
    gr_computedraw.bind();
    gr_computedraw.uniform1i("field_velocity_pressure", 0);
    gr_computedraw.uniform1i("screen_texture", 1);
    gr_computedraw.uniform2iv("ku_simdims", cvt.begin());
...
static void simulationSingleIteration()
{
    ...
    // u32 jacobiCounter = kg_jacobiSolverIterations - 58;
    ...
    gr_computeboundary.bind();
    gr_computeboundary.uniform1i("sim_velocity_pressure", 0);
    gr_computeboundary.uniform1i("sim_output", 1);
    gr_computeboundary.uniform2iv("ku_simdims", cvt.begin());
    gl::glBindTextureUnit(0, gr_vel_pressure);
    ...
    /* Shader step 0: compute advection and external forces */
    /* result written in gr_scratch_tex0 */
    gr_compute0.bind();
    gr_compute0.uniform1i("sim_velocity_pressure", 0);
    gr_compute0.uniform1i("sim_forces", 1);
    gr_compute0.uniform1i("sim_output", 2);
    gr_compute0.uniform1f("ku_dt", g_dt);
    gr_compute0.uniform1f("ku_viscosity", g_viscosity);
    ...
    /* shader step 1: compute diffusion. requires ping-pong between buffers until desired output is produced */
    /* result written finally in gr_diffusion */
    gr_compute1.bind();
    gr_compute1.uniform1i("sim_velocity_pressure", 0);
    gr_compute1.uniform1i("sim_output", 1);
    ...
    /* Shader step 2: compute field w */
    /* result written to gr_scratch_tex1 */
    gr_compute2.bind();
    gr_compute2.uniform1i("sim_forces_advection_pressure_terms", 0);
    gr_compute2.uniform1i("sim_diffusion_term", 1);
    gr_compute2.uniform1i("sim_output", 2);
    gr_compute2.uniform2iv("ku_simdims", cvt.begin());
    gl::glBindTextureUnit(0, gr_scratch_tex0);
    gl::glBindTextureUnit(1, gr_diffusion);
    gl::glBindImageTexture(2, gr_scratch_tex1, 0, false, 0, gl::GL_WRITE_ONLY, gl::GL_RGBA32F);
    gl::glDispatchCompute(g_dims.x, g_dims.y, 1);
    gl::glMemoryBarrier(shaderpassMask);


    /* Shader step 3: compute div(field_w) */
    /* result written to .w component of gr_fieldw; format now is [w.x, w.y, p, div(w)] */
    gr_compute3.bind();
    gr_compute3.uniform1i("sim_fieldw", 0);
    gr_compute3.uniform1i("sim_output", 1);
    gr_compute3.uniform2iv("ku_simdims", cvt.begin());
    gl::glBindTextureUnit(0, gr_scratch_tex1);
    gl::glBindImageTexture(1, gr_fieldw, 0, false, 0, gl::GL_WRITE_ONLY, gl::GL_RGBA32F);
    gl::glDispatchCompute(g_dims.x, g_dims.y, 1);
    gl::glMemoryBarrier(shaderpassMask);


    /* Shader step 4: compute new pressure */
    /* result written to .z component of gr_fieldw; format now is [w.x, w.y, p, div(w)] */
    gr_compute4.bind();
    gr_compute4.uniform1i("sim_w_p_divw", 0);
    gr_compute4.uniform1i("sim_output", 1);
    gr_compute4.uniform1f("ku_dt", g_dt);
    gr_compute4.uniform1f("ku_viscosity", g_viscosity);
    gr_compute4.uniform2iv("ku_simdims", cvt.begin());
    pingping_texid[0] = gr_fieldw;
    pingping_texid[1] = gr_scratch_tex1;
    jacobiCounter = kg_jacobiSolverIterations + 1;
    while(jacobiCounter > 1) {
        gl::glBindTextureUnit(0, pingping_texid[0]);
        gl::glBindImageTexture(1, pingping_texid[1], 0, false, 0, gl::GL_WRITE_ONLY, gl::GL_RGBA32F);
        gl::glDispatchCompute(g_dims.x, g_dims.y, 1);
    ...
    /* Shader step 5: compute the new velocity field */
    /* result written in gr_vel_pressure */
    gr_compute5.bind();
    gr_compute5.uniform1i("sim_w_p", 0);
    gr_compute5.uniform1i("sim_output", 1);
    gr_compute5.uniform2iv("ku_simdims", cvt.begin());
    gl::glBindTextureUnit(0, gr_fieldw);
    gl::glBindImageTexture(1, gr_vel_pressure, 0, false, 0, gl::GL_WRITE_ONLY, gl::GL_RGBA32F);
    gl::glDispatchCompute(g_dims.x, g_dims.y, 1);
    gl::glMemoryBarrier(gl::MemoryBarrierMask::GL_ALL_BARRIER_BITS);
