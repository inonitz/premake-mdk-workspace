...
static constexpr const char* computeShaderFilename[9] = {
    "projects/program/source/15optimize/0externalforce.comp",
    "projects/program/source/15optimize/1advection.comp",
    "projects/program/source/15optimize/2double_diffusion.comp",
    "projects/program/source/15optimize/3addition.comp",
    "projects/program/source/15optimize/4divergence.comp",
    "projects/program/source/15optimize/5diffusion2_new.comp",
    "projects/program/source/15optimize/6velocity.comp",
    "projects/program/source/15optimize/7draw.comp",
    "projects/program/source/15optimize/parallelReductionSSBO.comp"
};
...
util2::Time::Timestamp& optimize::getFrameTime()  { return g_frameTime; }
util2::Time::Timestamp& optimize::getRenderTime() { return g_renderTime; }
util2::Time::Timestamp& optimize::getTimer0() { return g_measuremisc0; }
util2::Time::Timestamp& optimize::getTimer1() { return g_measuremisc1; }
...
void optimize::initializeGraphics()
{
    ...
    for(u32 i = 0; i < __carraysize(g_buffer); ++i) {
        ...
        g_mappedBuffer[i] = gl::glMapNamedBufferRange(buf, 
...
static u32 compute_fluid()
{
    ...
    // UTIL2_TIME_NAMESPACE_MEASURE_CODE_BLOCK(util2::Time::getGeneralPurposeStamp(4), gl::glFinish());
...
static void compute_cfl(u32 texture)
{
    auto LockBuffer = [](gl::GLsync& sync) {
        if(sync) {
    ...
    auto WaitForBuffer = [](gl::GLsync& sync) {
        if(sync) {
            gl::GLenum waitForGPU;
            bool wait = true;
            while(wait) {
                waitForGPU = gl::glClientWaitSync(sync, gl::GL_SYNC_FLUSH_COMMANDS_BIT, 1);
                wait = (waitForGPU == gl::GL_ALREADY_SIGNALED) || (waitForGPU == gl::GL_CONDITION_SATISFIED);
            }
            return;
        }
    };
    ...
    */
    ...
    char* vecstr = __rcast(char*, std::malloc(256));
    if(g_mousePressed && awc2isMouseButtonPressed(AWC2_MOUSEBUTTON_RIGHT)) {
        for(i32 i = 0; i < g_reductionBufferLength; ++i) {
            const auto* maxMemMapBuf = __rcast(vec4f*, g_mappedBuffer[0]);
            maxMemMapBuf[i].to_strbuf(vecstr, 256);
            printf("  %u => %s\n", i, vecstr);
        }
    }
    std::free(vecstr);
    ...
    for(i32 i = 0; i < g_reductionBufferLength; ++i) {
        const auto* maxMemMapBuf = __rcast(vec4f*, g_mappedBuffer[0]);
        bool greaterThanX = boolean(maxMemMapBuf[i].x > g_velocityCFL.x);
        bool greaterThanY = boolean(maxMemMapBuf[i].y > g_velocityCFL.y);
        g_velocityCFL.x = __scast(f32, greaterThanX * maxMemMapBuf[i].x + !greaterThanX * g_velocityCFL.x);
        g_velocityCFL.y = __scast(f32, greaterThanY * maxMemMapBuf[i].y + !greaterThanY * g_velocityCFL.y);
    }
