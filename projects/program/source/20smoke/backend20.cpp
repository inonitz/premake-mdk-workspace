...
static void compute_smoke_quantities(
    u32 velocityPressureField, 
    u32 previousIteration,
    u32 nextIteration
);
...
u32 smoke20::compute_fluid()
{
    ...
    if(g_frameCounter % 2) {
        ...
        previousIterationQtty = gr_smokeTexture1;
        nextIterationQtty     = gr_smokeTexture0;
    } else {
        ...
        previousIterationQtty = gr_smokeTexture0;
        nextIterationQtty     = gr_smokeTexture1;
    }


    UTIL2_TIME_NAMESPACE_MEASURE_CODE_BLOCK(g_computeVelTime,   
        compute_velocity(previousIterationVel, nextIterationVel)
    );
    UTIL2_TIME_NAMESPACE_MEASURE_CODE_BLOCK(g_computeDyeTime,   
        compute_dye(
            nextIterationVel, 
    ...
    UTIL2_TIME_NAMESPACE_MEASURE_CODE_BLOCK(g_computeSmokeTime, 
        compute_smoke_quantities(
            nextIterationVel, 
            previousIterationQtty, 
            nextIterationQtty
        )
    );
    ...
    switch(g_chooseTextureToRender) {
        ...
        case 5: resultTex = nextIterationQtty; break;
        default: 
        break;
    }
    return resultTex;
...
static void compute_velocity(u32 previousIteration, u32 nextIteration)
{
    ...
    gr_computeInteractive.uniform1i("optionalBuoyancy", 2);
    ...
    gr_computeInteractive.uniform1f("ku_densityFactor",      g_densityFactor);
    gr_computeInteractive.uniform1f("ku_temperatureFactor",  g_temperatureFactor);
    gr_computeInteractive.uniform1f("ku_ambientTemperature", g_ambientTemperature);
    ...
    gr_computeInteractive.uniform1ui("ku_useBuoyancy",    g_useBuoyancy);
...
static void compute_smoke_quantities(
    ...
    u32 textureInput, textureOutput, tmp;


    gr_computeSmokeQuantities.bind();
    gr_computeSmokeQuantities.uniform1i("previousFrame", 0);
    gr_computeSmokeQuantities.uniform1i("velocityField", 1);
    gr_computeSmokeQuantities.uniform1i("nextFrame",     2);
    gr_computeSmokeQuantities.uniform1ui("ku_iteration", 1);
    gr_computeSmokeQuantities.uniform2iv("ku_simdims",      g_dims.begin());
    gr_computeSmokeQuantities.uniform2fv("ku_simUnitCoord", g_simUnitCoords.begin());
    gr_computeSmokeQuantities.uniform1f("ku_dt",       g_normdt);
    gr_computeSmokeQuantities.uniform1f("ku_alpha",    g_thermalDiffusivity);
    gl::glBindTextureUnit(0, previousIteration);
    gl::glBindTextureUnit(1, velocityPressureField);
    gl::glBindImageTexture(2, nextIteration, 0, false, 0, 
        gl::GL_WRITE_ONLY, 
        gl::GL_RGBA32F
    );
    gl::glDispatchCompute(g_computeInvocationSize.x, g_computeInvocationSize.y, g_computeInvocationSize.z);
    ...
    gr_computeSmokeQuantities.uniform1ui("ku_iteration", 2);
    textureInput  = nextIteration;
    textureOutput = gr_tmpTexture0;
    i32 i = 0;
    while(i < g_maximumJacobiIterations) {
        ...
        gl::glDispatchCompute(
            g_computeInvocationSize.x, 
            g_computeInvocationSize.y, 
            g_computeInvocationSize.z
        );
...
static void compute_dye(
    ...
    gr_computeInteractive.uniform1ui("ku_useBuoyancy",    false);
