#include "compute_buffer_mouse_advection_diffusion.hpp"
#include <threads.h>
...
__unused static inline void initializeVectorFields(vec2u const& dims, std::vector<vec4f>& field) {
    mark();
    ...
    static constexpr const char* computeShaderFilename[5] = {
        "projects/program/source/8compute_buffer_mouse_advection_diffusion/compute_buffer_mouse_advection_diffusion0.comp",
        "projects/program/source/8compute_buffer_mouse_advection_diffusion/compute_buffer_mouse_advection_diffusion1.comp",
        "projects/program/source/8compute_buffer_mouse_advection_diffusion/compute_buffer_mouse_advection_diffusion2.comp",
        "projects/program/source/8compute_buffer_mouse_advection_diffusion/compute_buffer_mouse_advection_diffusion3.comp",
        "projects/program/source/8compute_buffer_mouse_advection_diffusion/compute_buffer_mouse_advection_diffusion4.comp"
    };
#elif defined _WIN32
    static constexpr const char* computeShaderFilename[5] = {
        "C:/CTools/Projects/main/projects/program/source/8compute_buffer_mouse_advection_diffusion/compute_buffer_mouse_advection_diffusion0.comp",
        "C:/CTools/Projects/main/projects/program/source/8compute_buffer_mouse_advection_diffusion/compute_buffer_mouse_advection_diffusion1.comp",
        "C:/CTools/Projects/main/projects/program/source/8compute_buffer_mouse_advection_diffusion/compute_buffer_mouse_advection_diffusion2.comp",
        "C:/CTools/Projects/main/projects/program/source/8compute_buffer_mouse_advection_diffusion/compute_buffer_mouse_advection_diffusion3.comp",
        "C:/CTools/Projects/main/projects/program/source/8compute_buffer_mouse_advection_diffusion/compute_buffer_mouse_advection_diffusion4.comp"
    };
#endif
static std::vector<vec4f> g_initialField;
static vec2u              g_dims{1024, 1024};
static f32                g_dt        = 0.01f;
static f32                g_viscosity = 0.2f;
static u32                g_maximumJacobiIterations = 40;
static u32                g_texture[5];
static u32                g_fbo;
...
static f32                g_splatterRadius{0.5};
...
static u32&             gr_pingpongTexture0 = g_texture[1];
static u32&             gr_pingpongTexture1 = g_texture[2];
...
",
    ...
    ImGui::DragFloat("Viscosity Coefficient", &g_viscosity, 0.05f, 0.01f, 1.0f);
    ImGui::End();
...
static inline void render()
{
    ...
    if(!(g_frameCounter % 2)) {
        previousIteration = gr_pingpongTexture1;
        nextIteration     = gr_pingpongTexture0;
    } else {
        previousIteration = gr_pingpongTexture0;
        nextIteration     = gr_pingpongTexture1;
    }
    gr_tmpTexture0 = g_texture[3];
    gr_tmpTexture1 = g_texture[4];


    /* write some shit to a texture */
    ...
    gl::glBindImageTexture(1, gr_tmpTexture0, 0, false, 0, 
        gl::GL_WRITE_ONLY, 
        gl::GL_RGBA32F
    );
    ...
    /* apply advection to whatever we got in gr_tmpTexture0, write it to some other texture */
    ...
    gr_computeAdvection.uniform2iv("ku_simdims", cvt.begin());
    gl::glBindTextureUnit(0, gr_tmpTexture0);
    gl::glBindImageTexture(1, gr_tmpTexture1, 0, false, 0, 
        gl::GL_WRITE_ONLY, 
        gl::GL_RGBA32F
    );
    gl::glDispatchCompute(g_dims.x, g_dims.y, 1);
    ...
    /* apply diffusion to whatever we got in gr_tmpTexture0, write it to some other texture */
    ...
    gr_computeDiffusion.uniform2iv("ku_simdims", cvt.begin());
    textureInput  = gr_tmpTexture0;
    textureOutput = nextIteration;
    for(u32 i = 0; i < g_maximumJacobiIterations; ++i) {
    ...
    gr_computeAdd.bind();
    gr_computeAdd.uniform1i("advectedField", 0);
    gr_computeAdd.uniform1i("diffusedField", 1);
    gr_computeAdd.uniform1i("outputField",   2);
    gl::glBindTextureUnit(0, gr_tmpTexture0);
    gl::glBindTextureUnit(0, gr_tmpTexture1);
...
i32 compute_buffer_mouse_but_with_advection_and_diffusion()
{
