...
void makebelievebound25::render()
{
    // static const vec4f defaultScreenColor = vec4f{1.0f};
    static const vec4f defaultScreenColor = vec4f{0.0f};
...
";
    ...
    ImGui::SameLine();
    if(ImGui::Checkbox("Boundary", &g_imGuiButton[10]))
    {
        if(g_imGuiButton[10]) {
            g_imGuiButton[7] = 0;
            g_imGuiButton[8] = 0;
        ...
        mark();
        g_chooseUserDrawFillType = g_imGuiButton[10] ? USER_DRAW_FILL_TYPE_BOUNDARY : g_chooseUserDrawFillType;
    }
    ...
    ImGui::PopID();
    ImGui::SameLine();
    if(ImGui::Checkbox("Boundaries", &g_imGuiButton[2])) {
        if(g_imGuiButton[2]) {
            g_imGuiButton[0] = 0;
            g_imGuiButton[1] = 0;
            g_imGuiButton[3] = 0;
