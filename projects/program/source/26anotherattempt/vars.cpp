...
const char* computeShaderFilename[11] = {
    "projects/program/source/26anotherattempt/shaders/boundarypass.comp",
    "projects/program/source/26anotherattempt/shaders/0force.comp",
    "projects/program/source/26anotherattempt/shaders/1advect.comp",
    "projects/program/source/26anotherattempt/shaders/2diffusevel.comp",
    "projects/program/source/26anotherattempt/shaders/3div.comp",
    "projects/program/source/26anotherattempt/shaders/4diffusep.comp",
    "projects/program/source/26anotherattempt/shaders/5final.comp",
    "projects/program/source/26anotherattempt/shaders/6draw.comp",
    "projects/program/source/26anotherattempt/shaders/minmax.comp",
    "projects/program/source/26anotherattempt/shaders/errortex.comp",
    "projects/program/source/26anotherattempt/shaders/error_reduct.comp",
};
...
bool  g_useShaderNoBoundaries[2]{true, true};
bool  g_imGuiButton[20]{false};
u8    g_runCodeOnce[3]{false};
...
ShaderProgramV2& gr_computeBoundaries        = g_compute[0];
ShaderProgramV2& gr_computeInteractive       = g_compute[1];
ShaderProgramV2& gr_computeAdvection         = g_compute[2];
ShaderProgramV2& gr_computeDiffusionVel      = g_compute[3];
ShaderProgramV2& gr_computeDivergence        = g_compute[4];
ShaderProgramV2& gr_computeDiffusionPressure = g_compute[5];
ShaderProgramV2& gr_computeNewVelocity       = g_compute[6];
ShaderProgramV2& gr_computeRenderToTex       = g_compute[7];
ShaderProgramV2& gr_computeCFLCondition      = g_compute[8];
ShaderProgramV2& gr_computeErrorTexture      = g_compute[9];
ShaderProgramV2& gr_computeErrorEstimates    = g_compute[10];
...
u32 gr_prevIterBounds = DEFAULT32;
u32 gr_nextIterBounds = DEFAULT32;
u32 gr_bndQtyVel      = DEFAULT32;
u32 gr_bndQtyPressure = DEFAULT32;
...
void clearSimulationTextures()
{
    ...
    for(auto& tex : g_texture3d) {
        gl::glClearTexImage(tex, 0, gl::GL_RGBA, gl::GL_FLOAT, nullptr);
    }
    gl::glTextureSubImage2D(gr_bndryTexture0, 0, 
        0, 0, g_dims.x, g_dims.y, 
        gl::GL_RGBA, gl::GL_FLOAT, 
        g_boundaryTextureData.m_buf
    );
    gl::glTextureSubImage2D(gr_bndryTexture1, 0, 
        0, 0, g_dims.x, g_dims.y, 
        gl::GL_RGBA, gl::GL_FLOAT, 
        g_boundaryTextureData.m_buf
    );
...
void initializeGraphics()
{
    ...
    gr_prevIterBounds = gr_bndryTexture0;
    gr_nextIterBounds = gr_bndryTexture1;
    gr_bndQtyVel      = gr_bndQuantity0;
    gr_bndQtyPressure = gr_bndQuantity1;
    ...
    for(u32 i = 0; i < __carraysize(g_texture3d); ++i) {
        auto& tex = g_texture3d[i];
        gl::glTextureParameteri(tex, gl::GL_TEXTURE_WRAP_S, gl::GL_CLAMP_TO_EDGE);
        gl::glTextureParameteri(tex, gl::GL_TEXTURE_WRAP_T, gl::GL_CLAMP_TO_EDGE);
        gl::glTextureParameteri(tex, gl::GL_TEXTURE_WRAP_R, gl::GL_CLAMP_TO_EDGE);
        gl::glTextureParameteri(tex, gl::GL_TEXTURE_MIN_FILTER, gl::GL_NEAREST);
        gl::glTextureParameteri(tex, gl::GL_TEXTURE_MAG_FILTER, gl::GL_NEAREST);
        gl::glTextureStorage3D(tex, 1, gl::GL_RGBA32F, g_dims.x, g_dims.y, 4);
        gl::glClearTexImage(tex, 0, gl::GL_RGBA, gl::GL_FLOAT, nullptr);
    }
